...
# File processing
In general, I take a CSV file as input (almost always from VertNet) and process each record in the CSV file. There are fields that are targeted for parsing (finding traits) and other fields that are just carried through to the output as is. The following VertNet fields are usually targets for parsing.
...
1. [Tokenize the text.](#Tokenize-the-text)
2. (Optional) [Replace sequences of tokens to simplify the token stream.](#Simplify-tokens) Repeat this step as many times as needed.
3. [Convert sequences of tokens into the final productions.](#Create-traits-from-tokens)
...
**A drawback of pattern matching is that it tends to have a great precision but poor recall.**
...
### Simplify tokens
(Optional) Replace sequences of tokens to simplify the token stream. Repeat this step as many times as needed.
...
# Tests
Having a test suite is absolutely critical. The strategy I use is every new pattern gets its own test. Any time there is a parser error I add the parts that caused the error to the test suite and correct the parser. Most test are targeted at the resulting traits.
