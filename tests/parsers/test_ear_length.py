...
class TestEarLength(unittest.TestCase):
    def test_parse_01(self):
        self.assertEqual(
            ...
            [Trait(value=9, units="mm", units_inferred=False, start=0, end=16)],
        )

    def test_parse_02(self):
        self.assertEqual(
            EAR_LENGTH.parse('L. 9", T. 4", ear 9/16"'),
            [Trait(value=14.29, units='"', units_inferred=False, start=14, end=23)],
        )
    ...
    def test_parse_04(self):
        self.assertEqual(
            EAR_LENGTH.parse(
                '{"measurements":"TotalLength=180 Tail=82 ' 'HindFoot=28 Ear=18" }'
            ),
            [Trait(value=18, units=None, units_inferred=True, start=53, end=59)],
        )

    def test_parse_05(self):
        self.assertEqual(
            EAR_LENGTH.parse('{"earLength":"13", "gonadLength":"3"}'),
            [Trait(value=13, units=None, units_inferred=True, start=2, end=16)],
        )
    ...
    def test_parse_07(self):
        self.assertEqual(
            EAR_LENGTH.parse("verbatim collector=E. E. Makela 2432 ; sex=female"), []
        )
    ...
    def test_parse_23(self):
        self.assertEqual(EAR_LENGTH.parse("Cert #E0554185; Skull Seal #0406307"), [])

    def test_parse_24(self):
        self.assertEqual(EAR_LENGTH.parse("T=12x8, E379700/N3740240"), [])

    def test_parse_25(self):
        self.assertEqual(EAR_LENGTH.parse("Hawaiian chain.  Magnemite 610-E 7050."), [])
